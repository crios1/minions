...
Mutable Pipeline state belongs on that shared instance, normally initialized in `startup`, rather than in mutable class attributes. See {ref}`component-state-ownership` for the distinction between class, component, orchestration, and workflow state.
