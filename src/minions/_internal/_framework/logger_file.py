...
class FileLogger(Logger):
    """
    An async-aware structured logger that writes JSONL logs to disk with
    rotation and optional retention.

    Design Goals:
    - **Structured Logging**: Logs are written as newline-delimited JSON
      (JSONL), making them easy to parse with tools like `jq`, `grep`, or to
      ingest into log shippers and observability stacks.
    - **Single-Process Simplicity**: The logger avoids external dependencies
      and integrates cleanly into a single Python process, using
      `asyncio.to_thread()` to perform file writes without blocking the event
      loop.
    - **Log Rotation**: When the active log file exceeds a configured size
      (`max_log_file_bytes`), it is renamed with a filesystem-safe timestamp
    ...
    This logger is designed to be the default for the Minions framework,
    requiring no additional configuration to provide usable, structured logs
    out of the box.
    """
    ...
    async def log(self, level: int, msg: str, **kwargs: Any):
        try:
            ...
            if self._stdout:
                ...
                extras = " ".join(
                    f"{k}={_serialize_log_json(v) if not isinstance(v, str) else v}"
