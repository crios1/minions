...
async def test_gru_template2(
    ...
    # Example scenario (adjust as needed)
    starts: list[Start] = [
        Start(
            minion_modpath="tests.assets.minion_simple",
            pipeline_modpath="tests.assets.pipeline_simple_single_event_1",
    ...
    # Imports local to the test so they’re obvious and co-located
    from tests.assets.support.minion_spied import SpiedMinion
    from tests.assets.support.pipeline_spied import SpiedPipeline

    TIMEOUT = 3  # one place to tweak
    PIPELINE_EXPECTED_CALLS_PER_EVENT = {"__init__": 1, "produce_event": 1}

    # --- Helpers (local, tiny, readable) -------------------------------------
    def get_and_enable_spy_minion(modpath: str) -> type[SpiedMinion]:
        cls = gru._get_minion_class(modpath)
        assert issubclass(cls, SpiedMinion), f"{modpath} is not a SpiedMinion"
        cls.enable_spy()
        return cls

    def get_and_enable_spy_pipeline(modpath: str) -> type[SpiedPipeline]:
        cls = gru._get_pipeline_class(modpath)
        assert issubclass(cls, SpiedPipeline), f"{modpath} is not a SpiedPipeline"
        cls.enable_spy()
        return cls

    # Resolve unique classes once; keep handles for reset()
    unique_minion_classes: dict[str, type[SpiedMinion]] = {}
    unique_pipeline_classes: dict[str, type[SpiedPipeline]] = {}

    try:
        # --- Spy enable / class resolution -----------------------------------
        for s in starts:
            if s.minion_modpath not in unique_minion_classes:
                unique_minion_classes[s.minion_modpath] = get_and_enable_spy_minion(s.minion_modpath)
            if s.pipeline_modpath not in unique_pipeline_classes:
                unique_pipeline_classes[s.pipeline_modpath] = get_and_enable_spy_pipeline(s.pipeline_modpath)

        # --- Exercise ---------------------------------------------------------
        results = await asyncio.gather(*(gru.start_minion(**s.kwargs) for s in starts))

        # --- Basic result assertions -----------------------------------------
        for s, result in zip(starts, results):
            assert result.success == s.expect_success, f"result.success != {s.expect_success} for kwargs={s.kwargs}"

        # Count successful starts per *minion class* (not per modpath string)
        success_counts: defaultdict[type[SpiedMinion], int] = defaultdict(int)
        for s, result in zip(starts, results):
            if result.success:
                success_counts[unique_minion_classes[s.minion_modpath]] += 1

        total_successes = sum(success_counts.values())

        # --- Build expectations in one place (so the checks read like a spec) -
        # Pipelines: we currently assume each pipeline emits one event per start.
        # If that changes later, make this a function of the scenario.
        async def expect_pipeline_calls():
            await asyncio.gather(
                *[
                    pipeline_cls.wait_for_calls(expected=PIPELINE_EXPECTED_CALLS_PER_EVENT, timeout=TIMEOUT)
                    for pipeline_cls in unique_pipeline_classes.values()
                ]
            )

        # Minions: each workflow step should run once per successful start.
        async def expect_minion_step_calls():
            await asyncio.gather(
                *[
                    minion_cls.wait_for_calls(
                        expected={step_name: success_counts[minion_cls] for step_name in minion_cls._mn_workflow_spec},  # type: ignore[attr-defined]
                        timeout=TIMEOUT,
                    )
                    for minion_cls in unique_minion_classes.values()
                ]
        ...
        # State store: per successful workflow we expect N saves (one per step) and one delete at the end.
        async def expect_state_store_calls():
            await asyncio.gather(
                *[
                    state_store.wait_for_call(
                        "save_context",
                        count=len(minion_cls._mn_workflow_spec) * success_counts[minion_cls],  # type: ignore[attr-defined]
                        timeout=TIMEOUT,
                    )
                    for minion_cls in unique_minion_classes.values()
                ],
                state_store.wait_for_call("delete_context", count=total_successes, timeout=TIMEOUT),
            )

        # --- Await “work done” checks ----------------------------------------
        await expect_pipeline_calls()
        await expect_minion_step_calls()
        await expect_state_store_calls()

        # --- Call-order assertions (instance-scoped) --------------------------
        for minion_cls, n in success_counts.items():
            assert getattr(minion_cls, "_mn_workflow_spec"), "workflow spec must not be empty"
            order = ["__init__", *list(minion_cls._mn_workflow_spec)]  # type: ignore[attr-defined]
            for i in range(1, n + 1):
                minion_cls.assert_call_order_for_instance(i, order)
...
async def test_gru_template3(
    ...
    async def start_and_validate_minions(starts: list[Start]):
        # TODO: refactor this without the pipeline emits 1 event assumption
        ...
        pipeline_sentries: list[asyncio.Task] = []
        ...
        try:
            ...
            for s in starts:
                if s.minion_modpath not in unique_minion_classes:
                    cls = gru._get_minion_class(s.minion_modpath)
                    assert issubclass(cls, SpiedMinion)
                    cls.enable_spy()
                    unique_minion_classes[s.minion_modpath] = cls
                
                if s.pipeline_modpath not in unique_pipeline_classes:
                    cls = gru._get_pipeline_class(s.pipeline_modpath)
                    assert issubclass(cls, SpiedPipeline)
                    cls.enable_spy()
                    unique_pipeline_classes[s.pipeline_modpath] = cls
            
            for pipeline_cls in unique_pipeline_classes.values():
                pipeline_sentries.append(asyncio.create_task(_enforce_single_event(pipeline_cls)))

            results = await asyncio.gather(
                *[gru.start_minion(**s.kwargs) for s in starts]
            )
            
            success_counts: defaultdict[type[SpiedMinion], int] = defaultdict(int) # successful_minion_start_counts: dict[type[SpiedMinion], int] = {}

            for s, r in zip(starts, results):
                assert r.success == s.expect_success, f"result.success != {s.expect_success} for kwargs={s.kwargs}"
                if r.success:
                    success_counts[unique_minion_classes[s.minion_modpath]] += 1

            # --- Define Call Counts and Orders, Then assert them in Validate Work ---
            call_counts: dict[object, dict[int, str]] = {}
            call_orders: dict[object, list[str]] = {}

            for p_name, p_cls in unique_pipeline_classes.items():
                call_counts[p_name] = {'__init__':1, 'produce_event': 1}
            ...
            await asyncio.gather(*[
                # Workflow steps run once per successful minion start assuming
                # no step fails, aborts the workflow, and pipelines only emit
                # a single event during their lifetimes.
                *[
                    minion_cls.wait_for_calls(
                        expected={
                            name: success_counts[minion_cls] # TODO: handle workflow abortions and step failures
                            for name in minion_cls._mn_workflow_spec # type: ignore
                        },
                        timeout=3
                    )
                ...
                # StateStore saves context once per successful workflow step
                *[
                    state_store.wait_for_call(
                        'save_context',
                        count=len(minion_cls._mn_workflow_spec)*success_counts[minion_cls], # type: ignore
                        timeout=3
                    )
                    for minion_cls in unique_minion_classes.values()
                ],
                # StateStore deletes context when workflow ends (succeeded, failed, aborted)
                *[
                    state_store.wait_for_call(
                        'delete_context',
                        count=sum(success_counts.values()),
                        timeout=3
                    )
                ],
            ])

            # --- Validate Work (Assert Call Counts and Order) ---

            for minion_cls in success_counts.keys():
                assert minion_cls._mn_workflow_spec, "workflow spec must not be empty"
                for i in range(1, success_counts[minion_cls] + 1):
                    minion_cls.assert_call_order_for_instance(
                        i, ['__init__', *list(minion_cls._mn_workflow_spec)]
                    )
            
            state_store.assert_call_order([
                '__init__',
                'load_all_contexts', 
            ])
