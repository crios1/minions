...
async def test_gru_template3(
    ...
    async def start_and_validate_minions(starts: list[Start]):
        ...
        try:
            ...
            # --- Define Call Counts and Orders, (Then Await and Valid them) ---
            call_counts: dict[type[SpyMixin], dict[str, int]] = {}
            call_orders: dict[type[SpyMixin], list[str]] = {}

            for p_cls in unique_pipeline_classes.values():
                call_counts[p_cls] = {'__init__':1, 'produce_event': 1}

            for m_cls in unique_minion_classes.values():
                # Workflow steps run once per successful minion start
                # assuming no step fails, aborts the workflow, and pipelines
                # only emit a single event during thier lifetimes.
                call_counts[m_cls] = {
                    name: success_counts[m_cls] for name in m_cls._mn_workflow_spec # type: ignore
                }
                # StateStore saves context once per successful workflow step.
                call_counts[type(state_store)] = {
                    'save_context': len(m_cls._mn_workflow_spec) * success_counts[m_cls] # type: ignore
                }
            ...
            call_counts[type(state_store)] = {
                '__init__': 1,
                'load_context': 1,
                'delete_context': sum(success_counts.values())
            }
