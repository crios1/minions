...
def _names_for_spy_instance_identity(
    ...
    return [
        ...
        if recorded_call.spy_instance_identity == spy_instance_identity
    ]
...
class ScenarioVerifier:
    ...
    def _assert_call_order(
        ...
        state_store_spy_instance_identity = state_store_cls.get_spy_instance_identity(
            self._state_store
        )
        should_assert_state_store_order = (
            state_store_spy_instance_identity is not None and state_store_cls in call_counts
        )
        if should_assert_state_store_order:
            assert state_store_spy_instance_identity is not None
            self._result.spy_instance_identities[state_store_cls].add(state_store_spy_instance_identity)
        ...
        for m_cls in spies.minions.values():
            ...
            for spy_instance_identity in self._result.spy_instance_identities.get(m_cls, set()):
                names = _names_for_spy_instance_identity(m_cls, spy_instance_identity)
        ...
        for pipeline_id, p_cls in spies.pipelines.items():
            expected_events = self._plan.pipeline_event_targets.get(pipeline_id)
            for spy_instance_identity in self._result.spy_instance_identities.get(p_cls, set()):
                names = _names_for_spy_instance_identity(p_cls, spy_instance_identity)
        ...
        for r_cls in spies.resources:
            for spy_instance_identity in self._result.spy_instance_identities.get(r_cls, set()):
                names = _names_for_spy_instance_identity(r_cls, spy_instance_identity)
                order = ["__init__", "startup", "run"]
                if "shutdown" in names:
                    order.append("shutdown")
        ...
        if should_assert_state_store_order:
            assert state_store_spy_instance_identity is not None
            names = _names_for_spy_instance_identity(
                state_store_cls,
                state_store_spy_instance_identity,
            )
            order = ["__init__", "startup"]
            if "shutdown" in names:
                order.append("shutdown")
            state_store_cls.assert_call_order_for_instance(state_store_spy_instance_identity, order)
