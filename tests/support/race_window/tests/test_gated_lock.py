...
@pytest.mark.asyncio
async def test_holds_production_code_until_progress_is_allowed():
    lock = GatedLock()
    completed = False
...
@pytest.mark.asyncio
async def test_releases_when_lock_holder_is_cancelled():
    lock = GatedLock()
...
@pytest.mark.asyncio
async def test_wait_times_out_before_lock_is_held():
    lock = GatedLock()
